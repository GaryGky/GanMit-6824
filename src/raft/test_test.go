...
func TestReElection2A(t *testing.T) {
	...
	printDisconnected(leader2, (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	...
	printConnected((leader1 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	printConnected(leader2)
	cfg.checkOneLeader()
...
loop:
	for try := 0; try < 5; try++ {
		...
		for i := 1; i < iters+2; i++ {
			...
			if term1 != term {
				// CurrentTerm changed while starting
				continue loop
			}
			if !ok {
